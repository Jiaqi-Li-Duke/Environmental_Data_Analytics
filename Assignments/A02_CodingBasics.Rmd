...
```{r}
# Create a sequence from 1 to 100 by 4
seq <- seq(1,100,4)
# Compute mean and median of the sequence
seqmean <- mean(seq)
seqmed <- median(seq)
seqmean;seqmed
...
```{r}
name <- c("Jack", "Jen", "Jim", "John") # character vector
score <- c(80, 60, 40, 70) # numeric vector
pass <- c(TRUE, TRUE, FALSE, TRUE) # logical vector
grade <- data.frame(name, score, pass)
names(grade) <- c("Name", "Score", "Pass")
```

9. QUESTION: How is this data frame different from a matrix?

> ANSWER: A data frame can contain columns with different modes, while a matrix is a two-dimension structure which can only contain elements of the same type.
...
pass1 <- passtest(score)
pass1
```

12. QUESTION: Which option of `if` and `else` vs. `ifelse` worked? Why?

> ANSWER: The `ifelse` statement worked because the `ifelse()` statement is vectorized and it can run through every element in the vector. However, the `if` and `else` statement can only evaluate the first element in the vector and give a warning.
