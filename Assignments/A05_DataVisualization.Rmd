...
P.plot <- ggplot(PeterPaul, aes(x = po4, y = tp_ug)) +
  ...
  ylab(expression("Total phosphorus (\U003BCg/L)")) +
...
nutrient.facet <- 
  ggplot(subset(PeterPaul.gathered, lakename == 'Peter Lake'), aes(x = sampledate, y = concentration, color = depth)) +
  geom_point(size = 1) +
  facet_wrap(vars(nutrient), nrow = 5, scales="free_y", strip.position="right", labeller = as_labeller(nutrient)) +
  scale_x_date(limits = as.Date(c("1991-01-01", "2016-12-31")), 
    date_breaks = "9 months", date_labels = "%b %y") +
  xlab("Sample Date") +
  ylab(expression("Concentration (\U003BCg/L)")) +
...
> Answer: geom_line function connects all the data points and shows the pattern of discharge variation. On the other hand, geom_smooth function only shows an overall relationship between date and discharge rate.
...
tox.plot <- ggplot(tox, aes(x = Chemical.Name, y = Conc..Mean..Std.)) +
  geom_point(aes(color =Chemical.Name)) + 
  xlab(element_blank()) +
  ylab("Concentration") +
