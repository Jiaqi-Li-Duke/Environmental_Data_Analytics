...
PeterPaul.chem.nutrients.summaries <- read.csv("./Data/Processed/NTL-LTER_Lake_Summary_PeterPaul_Processed.csv")
...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue") #Color command needs to be outside aes()
print(O3plot2)

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone, color = Ozone)) #Color by ozone, but unnecessary here
print(O3plot2)
...
PMplot.faceted2 <-
  ...
  geom_point() +
  facet_grid(Year ~ Site.Name) #(Site.Name ~ Year)
...
templot <- ggplot(subset(PeterPaul.chem.nutrients, depth = 0)) +
  geom_point(aes(x = sampledate, y = temperature_C, color = year4))
print(templot)
