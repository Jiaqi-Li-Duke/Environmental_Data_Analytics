...
shapiro.test(EPAair$Ozone) # p-value is super low, so it is not normally distributed.
...
O3.onesample # p-value is low, reject the null hypothesis
```

What information does the output give us? How might we report this information in a report?

> ANWSER: Ozone concentration in 2017-2018 were significantly lower than 50 (one sample t-test; t = -41.911, df = 1084, p < 0.0001)
...
var.test(EPAair$Ozone ~ EPAair$Site.Name) # The two variances are different from each other, since the 95% interval does not overlap 1.
...
O3.twosample # Reject the null hypothesis that the two means are equal to each other, because of the really small p-value. y = blackstone (38.48 + error) or bryson (35.18 + error)
O3.twosample$p.value

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Site.Name)
summary(O3.twosample2) # y = blackstone (38.48 + error), and bryson (38.48 - 3.30 +error)
```
...
```{r}
O3.onesample.wilcox <- wilcox.test(EPAair$Ozone, mu = 50, alternative = "less") # Avoid the assumption of normality, may be a better test if the data are really away from normality
O3.onesample.wilcox # Get a v-value
O3.twosample.wilcox <- wilcox.test(EPAair$Ozone ~ EPAair$Site.Name)
O3.twosample.wilcox # It only suggests that the two sites are different, but does not give a mean value
```
...
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) # The variances are not significantly different from each other (or equal to each other)

# Format as a GLM
PM2.5.anova <- lm(EPAair$PM2.5 ~ EPAair$Site.Name)
summary(PM2.5.anova) # 36.73 for Blackstone, 4.43 less for Bryson, and 3.25 for Triple
...
TukeyHSD(PM2.5.anova2) # Bryson city and blackstone, triple and blackstone are significantly different from each other, but not triple and bryson.
...
> ANSWER: Blackstone has the highest PM2.5 value, bryson and triple are not significantly different from each other (ANOVA; F = 16.16, df = 1898, p < 0.0001).
...
summary(TNanova.main) # 309 for Paul Lake and depth_id is 1
...
```{r}
TNanova.interaction <- aov(PeterPaul.nutrients$tn_ug ~ PeterPaul.nutrients$lakename * PeterPaul.nutrients$depth_id) #interaction effects
