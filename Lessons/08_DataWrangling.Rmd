...
NTL.nutrient.data.PeterPaul <- filter(NTL.nutrient.data, lakename == "Paul Lake" | lakename == "Peter Lake") #Filter chooses rows.
...
#If we want to remove a column, just select(NTL.phys.data.PeterPaul, -lakename)
#NTL.phys.data.PeterPaul.skinny <- select(NTL.phys.data.PeterPaul, -lakeid)  
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.skinny, "nutrient", "concentration", tn_ug:po4) # The dimension changes by gather, the table becomes longer and narrower.

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) # Remove NAs when certain nutrient is not recorded.
...
NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration) #Opposite of Gather function, seperate a column into several ones.

# Split components of cells into multiple columns
# Opposite of 'separate' is 'unite'
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.skinny, sampledate, c("Y", "m", "d")) #The separate function won't keep the original sampledate column.
class(NTL.nutrient.data.PeterPaul.dates$Y) # The new Y column is as character.
...
<<<<<<< HEAD
write.csv(NTL.phys.nutrient.data.PeterPaul, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv")
=======
...
>>>>>>> 9e61c6b29e8c788d570f18670dd27b0a5859bd54
...
NTL.phys.nutrient.data.PeterPaul <- mutate(NTL.phys.nutrient.data.PeterPaul, month = month(sampledate)) # Add a month column without changing the original date column.

# reorder columns to put month with the rest of the date variables
NTL.phys.nutrient.data.PeterPaul <- select(NTL.phys.nutrient.data.PeterPaul, lakename, year4, month, sampledate:po4) # Reorder the columns which puts month right after year4.
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>%
  group_by(lakename) %>%
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% # It only keeps the observations which these three columns are all not NAs.
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
write.csv(NTL.PeterPaul.summaries, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_Summary_PeterPaul_Processed.csv")
