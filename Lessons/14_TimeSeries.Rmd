...
mk.test(Peter.nutrients.surface$tp_ug) # It does not count for seasonality. p-value is low, meaning that there is a trend over time. z is positive, which means that there is a positive trend over time. When the z score is closer to zero, we have less confidence that there is any trend.
...
pettitt.test(Peter.nutrients.surface$tp_ug) # The change point happens in No.35, which means 1-34 is a group, and 35 to the end is another group.

# Run separate Mann-Kendall for each change point
mk.test(Peter.nutrients.surface$tp_ug[1:34]) # There is not a significant change.
mk.test(Peter.nutrients.surface$tp_ug[35:132]) # There is not a significant change.

# Is there a second change point?
pettitt.test(Peter.nutrients.surface$tp_ug[35:132]) # Result is 79, so changing point is 35+79

# Run another Mann-Kendall for the second change point
mk.test(Peter.nutrients.surface$tp_ug[35:113]) # Increasing trend
mk.test(Peter.nutrients.surface$tp_ug[114:132]) # Not a significant trend

# Run the same test for Paul Lake. 
mk.test(Paul.nutrients.surface$tp_ug) # Non-significant trend
pettitt.test(Paul.nutrients.surface$tp_ug)

# Add vertical lines to the original graph to represent change points
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  geom_vline(xintercept=as.Date('1993-05-26'), lty = 2) +
  geom_vline(xintercept=as.Date('1998-05-26'), color = "red") +
  scale_color_manual(values = c("#7fcdbb", "#253494"))
...
ggplot(USGS.flow.data.trimmed, aes(x = datetime, y = discharge.mean)) +
  ...
  #scale_y_log10() # Better visualize the data
```
...
> ANSWER: Linear is more reliable with environmental data.
...
USGS.flow.data.timeseries <- ts(USGS.flow.data.interpolated$Discharge, 
                                start = c(2009, 1) ,frequency = 12) # Seasonal MK test only works with timeseries

# Run a Seasonal Mann-Kendall test
USGS.smktest <- smk.test(USGS.flow.data.timeseries)
USGS.smktest # Larger the z-score, stronger the trend
summary(USGS.smktest) # Giving us trends for each month
```
