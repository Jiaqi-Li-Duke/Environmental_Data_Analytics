...
<Fill in our discussion below with bullet points. Use italics and bold for emphasis, this line does not show up in the R Markdown file>
...
---------------------- | ----------------------
EPAair_O3_NC2017_raw   | 2017 NC ozone concentration
EPAair_O3_NC2018_raw   | 2018 NC ozone concentration
ECOTOX_Neonicotinoids  | Ecotox dataset
...
nutri <- read.csv("../Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
head(nutri)
nutri$sampledate <- as.Date(nutri$sampledate, format = "%m/%d/%y")
```
...
TN <- nutri_tidy %>%
  filter(!is.na(tn_ug)) %>%
  group_by(lakename) %>%
  summarize("Mean" = mean(tn_ug), "Minimum" = min(tn_ug), "Maximum" = max(tn_ug), "SD" = sd(tn_ug))

TP <- nutri_tidy %>%
  filter(!is.na(tp_ug)) %>%
  group_by(lakename) %>%
  summarize("Mean" = mean(tp_ug), "Minimum" = min(tp_ug), "Maximum" = max(tp_ug), "SD" = sd(tp_ug))
...
```{r, echo=FALSE}
kable(TN, caption = "Total Nitrogen")
kable(TP, caption = "Total Phosphorus")
```
...
TN.plot <- ggplot(nutri_tidy) +
  geom_point(aes(x = sampledate, y = tn_ug, col = lakename)) +
...
TP.plot <- ggplot(nutri_tidy) +
  geom_point(aes(x = sampledate, y = tp_ug, col = lakename)) +
  scale_color_discrete(name = "Lake name") +
