...
summary(irradiance.regression) # irradiance = 487 - 96*depth + error (the error is a lot, because r-square is small)
...
> ANSWER: Irradiance is negatively correlated with depth (linear regression; R-square = 0.3091, df = 15449, p < 0.0001) (more detailed description with the equation is also fine; F-statistics is not necessary) 
...
plot(irradiance.regression) # Not what to include in the report, just check the output
```
...
```{r, fig.height = 3, fig.width = 4}
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, irradianceWater != 0) # To do the log-transform, remove the zero.
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth)
summary(irradiance.regression2) # log(irradiance) = 6.2 - 0.74*depth + error
plot(irradiance.regression2)

# Add a line and standard error for the linear regression
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  scale_y_log10() + # Represent log-transformed
  geom_point()  +
...
summary(TPregression) # TP = 6 + 1.5 * depth + 0.94 * DO + error
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit() # must omit NAs to use this function
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse") # square figure which has duplicates
...
step(TPAIC) # Start with the full model, lower AIC value, the better
...
summary(TNancova.main) # TN = 353 (Paul Lake, depth = 0) + 135.36 (if Peter) - 9.7 * depth, r-square is only 9%

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction) # TN = 325 (Paul Lake, depth = 0) + 185 (if Peter) +20*depth (if Paul) -48*depth (if peter) + error, the model is a little bit better (12%)
