...
> Answer: NAs in social science surveys may mean "do not apply", and NAs in water quality researches may mean below detection, which also means something.
...
```{r, fig.height = 3, fig.width = 4}
# Graph the counts, give a warning that using `bins = 30`. Pick better value with `binwidth`.
...
```{r, fig.height = 3, fig.width = 4}
# Useful to plot variables in the same scale
ggplot(USGS.flow.data.complete) +
...
# Color by whether approval or not
ggplot(USGS.flow.data.complete) +
