...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]# Comma means choosing all the columns
...
head(NTL.phys.data.surface1) # The row names remain the same
dim(NTL.phys.data.surface1)
head(NTL.phys.data.surface2) # The row names change
dim(NTL.phys.data.surface2)
head(NTL.phys.data.surface3) # The row names change
...
NTL.phys.data.Tues.1999 <- filter(NTL.phys.data, year4 >= 1990 & year4 <= 1999 & lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> ANSWER: It does not make sense for other users. And if we update our raw dataset, it is not reproducible.
...
NTL.phys.data.temp.ascending <- arrange(NTL.phys.data, desc(NTL.phys.data$temperature_C))
head(NTL.phys.data.temp.ascending,10)
```
### Select
...
NTL.phys.data.temps$temp_F <- (NTL.phys.data.temps$temperature_C*9/5) + 32
```
### Pipes
...
NTL.phys.data.processed <- 
  NTL.phys.data %>% # Data set to process at the first line
